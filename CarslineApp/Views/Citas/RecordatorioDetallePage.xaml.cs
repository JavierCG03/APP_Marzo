...
public partial class RecordatorioDetallePage : ContentPage
{
    ...
    private async Task CargarDetalleRecordatorio()
    {
        try
        {
            ...
            else
            {
                ...
                await DisplayAlert("Advertencia", response.Message ?? "No se encontr� el recordatorio", "OK");
    ...
    #region M�todos de Comunicaci�n
    ...
    private async void OnEnviarWhatsAppClicked(object sender, EventArgs e)
    {
        ...
        {
            await DisplayAlert("Advertencia", "No hay numero de tel�fono disponible", "OK");
            return;
        }
    ...
    private async void OnLlamarTelefonoCasaClicked(object sender, EventArgs e)
    {
        ...
        {
            await DisplayAlert("Advertencia", "No hay numero de tel�fono de casa disponible", "OK");
            return;
        }
    ...
    /// <summary>
    /// Enviar correo electr�nico con recordatorio
    /// </summary>
    ...
    private async Task RealizarLlamada(string telefono)
    {
        if (string.IsNullOrWhiteSpace(telefono))
        {
            await DisplayAlert("Advertencia", "No hay n�mero de tel�fono disponible", "OK");
            return;
        }
        ...
        try
        {
            ...
            {
                await DisplayAlert("? Error", "El dispositivo no soporta llamadas telef�nicas", "OK");
            }
        }
        catch (Exception ex)
        {
...
Le recomendamos agendar su cita para conservar su veh�culo en optimas condiciones{Final}
...
¡Gracias por su preferencia!";
    ...
    /// <summary>
    /// Limpia un n�mero de tel�fono quitando caracteres no num�ricos
    /// </summary>
    private string LimpiarNumeroTelefono(string telefono)
    {
        ...
        // Mantener el + si est� al inicio, quitar todo lo dem�s excepto n�meros
        var limpio = new string(telefono.Where(c => char.IsDigit(c) || c == '+').ToArray());

        // Asegurar que el + solo est� al inicio
        if (limpio.Contains('+'))
        {
